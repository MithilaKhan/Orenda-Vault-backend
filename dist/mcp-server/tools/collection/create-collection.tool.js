...
function createCollectionTool(server) {
    server.registerTool("createCollection", {
        ...
        try {
            const result = yield collection_service_1.CollectionService.createCollectionToDB({ title, description, icon: icon || "FileText", user: userId });
            return {
                content: [
                    {
                        type: "text",
